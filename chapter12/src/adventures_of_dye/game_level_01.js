...
load() {
    engine.text.load(this.kLevelFile);
...
unload() {
    ...
    engine.text.unload(this.kLevelFile);
...
initialize() {
   ...
   engine.defaultResources.setGlobalAmbientIntensity(0.2);
    ...
    engine.layer.addToLayer(engine.eActors, this.mIllumHero);
...
update() {
    ...
    for (i = 0; i < this.mAllMinions.size(); i++) {
        let minionBox = this.mAllMinions.getObjectAt(i).getPhysicsComponent();
    ...
    for (i = 0; i < this.mAllMinions.size(); i++) {
        ...
        for (j = 0; j < p.size(); j++) {
            let pBox = p.getObjectAt(j).getPhysicsComponent();
    ...
    for (i = 0; i < this.mAllButtons.size(); i++) {
        let buttonBox = this.mAllButtons.getObjectAt(i).getPhysicsComponent();
