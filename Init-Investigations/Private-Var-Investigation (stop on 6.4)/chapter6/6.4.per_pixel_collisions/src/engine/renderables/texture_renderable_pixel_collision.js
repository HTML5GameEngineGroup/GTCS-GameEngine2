...
TextureRenderable.prototype.setColorArray = function() {
    if (this.mColorArray === null) {
        this.mColorArray = texture.getColorArray(this._mTexture);
    }
}
...
TextureRenderable.prototype._pixelAlphaValue = function(x, y) {
    x = x * 4;
    y = y * 4;
    return this.mColorArray[(y * this._mTextureInfo.mWidth) + x + 3];
}
...
TextureRenderable.prototype._wcPositionToIndex = function(returnIndex, wcPos) {
    ...
    vec2.sub(delta, wcPos, xf.getPosition());
    returnIndex[0] = this.mElmWidthPixels * (delta[0] / xf.getWidth());
    returnIndex[1] = this.mElmHeightPixels * (delta[1] / xf.getHeight());
...
TextureRenderable.prototype._indexToWCPosition = function(returnWCPos, i, j) {
    let xf = this.getXform();
    let x = i * xf.getWidth() / this.mElmWidthPixels;
    let y = j * xf.getHeight() / this.mElmHeightPixels;
    returnWCPos[0] = xf.getXPos() + (x - (xf.getWidth() * 0.5));
    returnWCPos[1] = xf.getYPos() + (y - (xf.getHeight() * 0.5));
}
