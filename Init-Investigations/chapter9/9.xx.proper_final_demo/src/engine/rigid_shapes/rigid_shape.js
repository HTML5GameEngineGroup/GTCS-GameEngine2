...
class RigidShape {
    ...
    setMass(m) {
        if (m > 0) {
            ...
            //this.mVelocity = [0, 0];              <-- WHAT IS GOING ON HERE?
            //this.mAngularVelocity = 0;
        }
        this.updateInertia();
    }
    ...
    // NEED THIS LATER
    resolveParticleCollision(aParticle, xf) {
        let status = false;
        if (this.getType() === "RigidCircle") {
            status = particleSystem.resolveCirclePos(this, aParticle);
            return status;
        }
        else if (this.getType() === "RigidRectangle") {
            status = particleSystem.resolveRectPos(this, xf);
            return status;
        }
        else { return status; }
    }
