...
class MyGame extends engine.Scene {
    ...
    update() {
        ...
        if (engine.input.isKeyPressed(engine.input.keys.C)) {
            if (this.mCamera.isMouseInViewport()) {
                let p = this.createParticle(this.mCamera.mouseWCX(), this.mCamera.mouseWCY());
    ...
    createParticle(atX, atY) {
        ...
        let p = new ParticleGameObject(this.kParticleTexture, atX, atY, life);
        p.getRenderable().setColor([1, 0, 0, 1]);
        
        // size of the particle
        let r = 3.5 + Math.random() * 2.5;
        ...
        let fx = 10 * Math.random() - 20 * Math.random();
